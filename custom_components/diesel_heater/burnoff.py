...
class BurnoffController:
    ...
    def get_alias(self, name: str) -> Any:
        """Return a coordinator `_burnoff_*` field mapped onto this controller."""
        if name == "_burnoff_lock":
            return self.lock
        if name == "_burnoff_cancel_event":
            return self.cancel_event
        if name == "_burnoff_task":
            return self.task
        if name == "_burnoff_active":
            return self.active
        if name == "_burnoff_awaiting_snapshot_write":
            return self.cycle.phase == BurnoffPhase.RESTORING
        if name == "_burnoff_awaiting_first_status_after_reload":
            return self.cycle.phase == BurnoffPhase.AWAITING_STATUS
        if name == "_burnoff_shutdown_after":
            return self.cycle.shutdown_after
        if name == "_burnoff_ends_at":
            return self.cycle.ends_at
        if name == "_burnoff_saved_mode":
            return self.cycle.saved_mode
        if name == "_burnoff_saved_level":
            return self.cycle.saved_level
        if name == "_burnoff_saved_temp":
            return self.cycle.saved_temp
        if name == "_burnoff_applying":
            return self.cycle.applying
        if name == "_burnoff_heating_seconds":
            return self.accumulator.heating_seconds
        if name == "_burnoff_cycles":
            return self.accumulator.cycles
        if name == "_burnoff_pending":
            return self.accumulator.pending
        if name == "_burnoff_just_completed":
            return self.accumulator.just_completed
        if name == "_burnoff_prev_step":
            return self.accumulator.prev_step
        if name == "_burnoff_prev_state":
            return self.accumulator.prev_state
        if name == "_burnoff_prev_mode":
            return self.accumulator.prev_mode
        if name == "_burnoff_skip_pending_on_off":
            return self.skip_pending_on_off
        if name == "_burnoff_ha_power_off":
            return self.ha_power_off
        if name == "_burnoff_start_scheduled":
            return self.start_scheduled
        if name == "_burnoff_in_run_aborts":
            return self.accumulator.in_run_aborts
        if name == "_burnoff_skip_in_run":
            return self.accumulator.skip_in_run
        raise KeyError(name)

    def set_alias(self, name: str, value: Any) -> bool:
        """Assign a coordinator `_burnoff_*` field. Return False if unknown."""
        if name == "_burnoff_lock":
            self.lock = value
            return True
        if name == "_burnoff_cancel_event":
        ...
        if name == "_burnoff_active":
            if value:
                ...
                self.cycle.phase = BurnoffPhase.IDLE
            return True
        if name == "_burnoff_awaiting_snapshot_write":
            if value:
                self.cycle.phase = BurnoffPhase.RESTORING
            elif self.cycle.phase == BurnoffPhase.RESTORING:
                self.cycle.phase = BurnoffPhase.RUNNING
            return True
        if name == "_burnoff_awaiting_first_status_after_reload":
            if value:
                self.cycle.phase = BurnoffPhase.AWAITING_STATUS
            elif self.cycle.phase == BurnoffPhase.AWAITING_STATUS:
                self.cycle.phase = BurnoffPhase.RUNNING
            return True
        if name == "_burnoff_shutdown_after":
            self.cycle.shutdown_after = bool(value)
            return True
        if name == "_burnoff_ends_at":
            self.cycle.ends_at = value
            return True
        if name == "_burnoff_saved_mode":
            self.cycle.saved_mode = value
            return True
        if name == "_burnoff_saved_level":
            self.cycle.saved_level = value
            return True
        if name == "_burnoff_saved_temp":
            self.cycle.saved_temp = value
            return True
        if name == "_burnoff_applying":
            self.cycle.applying = bool(value)
            return True
        if name == "_burnoff_heating_seconds":
            self.accumulator.heating_seconds = float(value)
            return True
        if name == "_burnoff_cycles":
            self.accumulator.cycles = int(value)
            return True
        if name == "_burnoff_pending":
            self.accumulator.pending = bool(value)
            return True
        if name == "_burnoff_just_completed":
            self.accumulator.just_completed = bool(value)
            return True
        if name == "_burnoff_prev_step":
            self.accumulator.prev_step = value
            return True
        if name == "_burnoff_prev_state":
            self.accumulator.prev_state = value
            return True
        if name == "_burnoff_prev_mode":
            self.accumulator.prev_mode = value
            return True
        if name == "_burnoff_skip_pending_on_off":
            self.skip_pending_on_off = bool(value)
            return True
        if name == "_burnoff_ha_power_off":
            self.ha_power_off = bool(value)
            return True
        if name == "_burnoff_start_scheduled":
            self.start_scheduled = bool(value)
            return True
        if name == "_burnoff_in_run_aborts":
            self.accumulator.in_run_aborts = int(value)
            return True
        if name == "_burnoff_skip_in_run":
            self.accumulator.skip_in_run = bool(value)
            return True
        return False
